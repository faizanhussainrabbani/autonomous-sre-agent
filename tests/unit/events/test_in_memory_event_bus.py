...
async def test_event_bus_exact_subscription(
    event_bus: InMemoryEventBus, sample_event: DomainEvent
) -> None:
    received = []
...
async def test_event_bus_deduplication(
    ...
    """A handler subscribed to exact and wildcard should run once per event."""
    received = []
...
async def test_event_store_append_and_retrieve(
    event_store: InMemoryEventStore, sample_event: DomainEvent
) -> None:
