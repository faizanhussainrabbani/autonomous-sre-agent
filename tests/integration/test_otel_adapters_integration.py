...
from sre_agent.adapters.telemetry.otel.jaeger_adapter import JaegerTraceAdapter  # noqa: E402
from sre_agent.adapters.telemetry.otel.loki_adapter import LokiLogAdapter  # noqa: E402
from sre_agent.adapters.telemetry.otel.prometheus_adapter import (  # noqa: E402
...
def prometheus():
    with (
        DockerContainer("prom/prometheus:v2.48.0")
        .with_exposed_ports(9090)
        .with_command("--config.file=/etc/prometheus/prometheus.yml") as container
    ):
...
def jaeger():
    with (
        DockerContainer("jaegertracing/all-in-one:1.52")
        .with_exposed_ports(16686)
        .with_env("COLLECTOR_ZIPKIN_HOST_PORT", ":9411") as container
    ):
        host = container.get_container_host_ip()
