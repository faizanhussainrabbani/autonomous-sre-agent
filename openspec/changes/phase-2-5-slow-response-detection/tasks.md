...
- [x] **0.1** Confirm scope split in planning docs and team sync: Phase 2.5A (K8s + AWS), Phase 2.5B (Azure)
  - Output: recorded decision in phase tracking notes

- [x] **0.2** Confirm Azure adapter dependency status
  - Check whether `src/sre_agent/adapters/telemetry/azure/` exists and is production-ready
  - Output: `blocked` — no Azure adapter directory found; Phase 2.5B deferred

---

## Gate 1 — Core Domain Extensions (Phase 2.5A)

- [x] **1.1** Add `SLOW_RESPONSE` and `TIMEOUT_PROXIMITY` to `AnomalyType`
  - File: `src/sre_agent/domain/models/canonical.py`

- [x] **1.2** Extend `DetectionConfig` with:
...
- [x] **1.3** Wire config ingestion through settings/config files
  - Files: `src/sre_agent/config/settings.py`, `config/agent.yaml` (and env mapping location if used)

- [x] **1.4** Extend `set_service_sensitivity()` with `slow_response_threshold_ms`
...
- [x] **2.1** Implement `_detect_absolute_latency()`
  - File: `src/sre_agent/domain/detection/anomaly_detector.py`

- [x] **2.2** Implement `_detect_timeout_proximity()`
  - Include fallback behavior when timeout metadata is missing
  - File: `src/sre_agent/domain/detection/anomaly_detector.py`

- [x] **2.3** Refactor latency evaluation path to candidate-rule model
...
- [x] **2.4** Apply cold-start suppression to timeout-proximity rule
  - File: `src/sre_agent/domain/detection/anomaly_detector.py`
...
- [x] **3.1** Add ECS `ResponseTime` mapping
  - Canonical name: `ecs_response_time_ms`
  - File: `src/sre_agent/adapters/telemetry/cloudwatch/metrics_adapter.py`

- [x] **3.2** Enrich Lambda duration metrics with `timeout_ms` metadata
  - Source: `AWSResourceMetadataFetcher.fetch_lambda_context()`
  - File: `src/sre_agent/adapters/telemetry/cloudwatch/metrics_adapter.py`

- [x] **3.3** Add Kubernetes p99 latency metric to polling defaults/watchlist
...
- [x] **5.1** Add/update tests in `tests/unit/domain/test_detection.py` for:
...
- [x] **5.2** Add/update defaults tests in `tests/unit/domain/test_detection_config.py`

### Unit — Adapters/Config

- [x] **5.3** Add/update adapter tests in `tests/unit/adapters/test_cloudwatch_metrics_adapter.py`:
  - ECS `ResponseTime` mapping
  - Lambda timeout metadata enrichment

- [x] **5.4** Add/update settings tests in `tests/unit/config/test_settings.py` for new detection fields

### Integration/E2E

- [x] **5.5** Add `tests/e2e/test_slow_response_e2e.py` covering:
...
- [x] **6.1** All Gate 1–5 tasks complete
- [x] **6.2** New and existing tests pass for Phase 2.5A scope
- [x] **6.3** No regression in existing latency spike behavior
- [x] **6.4** Coverage remains at or above repository threshold
...
- [x] **8.1** Update architecture docs for hybrid slow-response detection
- [x] **8.2** Update `CHANGELOG.md` with Phase 2.5A and (if completed) 2.5B entries
- [x] **8.3** Update sample config docs with new thresholds and rule precedence behavior
