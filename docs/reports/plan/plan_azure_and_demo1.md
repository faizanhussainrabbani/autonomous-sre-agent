...
1. **Demo 1 (Missing Demo 1 - Basic Telemetry Fetch & Baseline)**: Establish the first demo in the sequence `../../../scripts/demo/live_demo_1_telemetry_baseline.py` to demonstrate the lowest level of Phase 1 — basic metric fetching and conversion to Canonical representation using existing LocalStack services.
2. **Demo 11 (Provider Coverage - Azure Operations)**: Create `../../../scripts/demo/live_demo_11_azure_operations.py` to exercise the Azure App Service and Azure Functions adapters from Phase 1.5, proving that the Cloud Operator abstraction seamlessly delegates to Azure models using mock Azure clients (since Azure LocalStack equivalents aren't natively available).
...
1. **Create** `../../../scripts/demo/live_demo_1_telemetry_baseline.py`: Use `setup_cloudwatch` and `CloudWatchMetricsAdapter` to fetch `PutMetricData` points and display `CanonicalMetric` objects.
2. **Create** `../../../scripts/demo/live_demo_11_azure_operations.py`: Instantiate `AppServiceOperator` and `FunctionsOperator` with mock `web_client` objects and invoke their `restart` and `scale` methods.
